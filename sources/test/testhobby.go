...
func (d *TestHobbySource) Get(ctx context.Context, scope string, query string) (*sdp.Item, error) {
	if scope != "test" {
...
func (d *TestHobbySource) List(ctx context.Context, scope string) ([]*sdp.Item, error) {
	if scope != "test" {
		return nil, &sdp.QueryError{
			ErrorType:   sdp.QueryError_NOSCOPE,
			ErrorString: "test queries only supported in 'test' scope",
...
func (d *TestHobbySource) Search(ctx context.Context, scope string, query string) ([]*sdp.Item, error) {
	if scope != "test" {
		return nil, &sdp.QueryError{
			ErrorType:   sdp.QueryError_NOSCOPE,
			ErrorString: "test queries only supported in 'test' scope",
